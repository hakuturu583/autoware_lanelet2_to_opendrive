...
"which places does this scenario name", so the editor can draw them on the map
it already renders, beside the timeline that says when.
...
class MapPoint:
    ...
    :attr:`kind`, :attr:`title` and :attr:`role` are the slot's own -- see
    :class:`~autoware_carla_scenario.authoring.models.LaneletSlot` -- so a
    primitive that declares a new lanelet field is drawn without this module
