...
(function () {
  ...
  document.addEventListener('click', function (event) {
    ...
    // Emptying a picked value: the map can set a lanelet but not unset one,
    // and a field that may be left blank -- the ego's goal -- needs a way back
    // to blank. It goes through the same `change` the picker dispatches, so
    // clearing and picking reach the server by one path.
